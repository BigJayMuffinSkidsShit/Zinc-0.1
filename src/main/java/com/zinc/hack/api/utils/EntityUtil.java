...
import com.zinc.hack.mixin.mixins.IEntityLivingBase;
...
public class EntityUtil
    ...
    public static boolean isPlayerValid(final EntityPlayer player, final float range) {
        return player != mc.player && mc.player.getDistance(player) < range && !isDead(player) && !Zinc.friendManager.is(player.getName());
    }
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && Zinc.friendManager.is(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && Zinc.friendManager.is(entity.getName())) {
    ...
    public static boolean isCrystalAtFeet(final EntityEnderCrystal crystal, final double range) {
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            if (Zinc.friendManager.is(player.getName())) {
                continue;
            }
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (Zinc.friendManager.is(player.getName()) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2\u00a7f0", (int) EntityUtil.mc.player.getDistance(player));
